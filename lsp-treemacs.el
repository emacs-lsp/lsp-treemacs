...
(defun lsp-treemacs--type-hierarchy-render-nodes (result loaded? &optional type)
  (-map (-lambda ((it &as &hash "name" "children" "parents" "kind" "uri" "range" (&hash "start")))
          `(:label ,(concat name (cl-case type
                                   (lsp-treemacs--hierarchy-sub (propertize " ↓" 'face 'shadow))
                                   (lsp-treemacs--hierarchy-super (propertize " ↑" 'face 'shadow))))
                   :key ,name
                   :icon ,(lsp-treemacs-symbol-kind->icon kind)
                   ,@(if loaded?
...
(defun lsp-treemacs--type-hierarchy-render (node type _ callback)
    ...
    (lsp-request-async
     ...
     (-lambda ((&hash "children" "parents"))
       (funcall callback (lsp-treemacs--type-hierarchy-render-nodes
                          (if (eq type lsp-treemacs--hierarchy-sub)
                              children
                            parents)
                          nil type))))))
    ...
    (if result
        (pop-to-buffer
         (lsp-treemacs-render
          ...
          (concat (cl-case direction
                    (lsp-treemacs--hierarchy-sub "Sub")
                    (lsp-treemacs--hierarchy-super "Super")
                    (lsp-treemacs--hierarchy-both "Sub/Super"))
                  " Type Hierarchy")
          nil
